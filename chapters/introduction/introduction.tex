...
The environment plays an important role. Galaxy over-density in groups leads to
...
\cite{Peng+10}'s measure of environment is essentially a local measure, and
can't trace global environment.
%
\remark{%
    Assuming the density profile of $p \left(r\right)$ \citet{NFW+97}, the
    over-density $\delta$ is:
    %
    \begin{equation}
    ...
    density fraction of matter in the Universe and $\rho_c$ is the critical
    density equal to $3 H_0^2/ \left(8\pi G\right)$, we finally have:
    %
    \begin{equation}
...
introduced voluntary or not in \bartrefchapter{mock}. We were also interested
...
\bartrefchapter{friends_of_friends_algorithm}. We present and test MAGGIE, a
probabilistic Bayesian galaxy group algorithm that reduces the effects of
interlopers in the galaxy group properties observed in \bartrefchapter{MAGGIE}.
In \bartrefchapter{sdss}, we describe our analysis of the Sloan Digital Sky
