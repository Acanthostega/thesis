...
\cs{Analysing data on the SDSS-DR8}
...
$2\pi\pg1-\cos\theta_{\mathrm{max}}\pd$. \com{It doesn't seem to be a standard definition for the area on the sky...}. I have made
...
\cs{How to generate mock catalogues?}
\cs{Calcul du barycentre lumineux des groupes\label{sec:annbar}}
\begin{quotation}
...
correspondantes.
\cs{Calcul de la fonction $\Gamma$ incomplète\label{sec:anngam}}
...
s'écrire comme:
\begin{eq}
        \Gamma(a,x)=\int_x^{\infty}e^{-t}{t^{a-1}}\dd{t}=\Gamma(a)\left(1-P(a,x)\right)
\end{eq}
avec\begin{eq}
        P(a+1,x)=P(a,x)-\frac{{x^a}{e^{-x}}}{\Gamma(a+1)}
\end{eq}
